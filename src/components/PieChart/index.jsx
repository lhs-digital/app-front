  ...
  useEffect(() => {
    const processData = async () => {
      ...
      try {
        ...
        const tableData = fetchedData.data.tables.find(
          (table) => table.label === selectedTable,
        );
        ...
        const sortedColumns = [...tableData.columns].sort(
          (a, b) => b.errors_count - a.errors_count,
        );
        ...
        const newChartsData = sortedColumns.map((column) => {
          ...
          const percentageErrors =
            total > 0 ? ((column.errors_count / total) * 100).toFixed(2) : 0;
          const percentageFixed =
            total > 0
              ? ((column.fixed_errors_count / total) * 100).toFixed(2)
              : 0;
